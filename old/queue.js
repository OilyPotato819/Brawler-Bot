...
const { QueueViewer } = require('../classes/queueViewer.js');
...
class Queue {
  ...
  async _playResource() {
    const stream = ytdl.exec(this.playing.url, {
      extractAudio: true,
      audioFormat: 'mp3',
      output: '-',
    });

    let resource = createAudioResource(stream.stdout);
  ...
  async _getSongs(input) {
    if (!input.contains('youtube.com') && !input.contains('youtu.be')) return;

    const videoId = input.match(/(?<=\/|v=)[-_A-Za-z0-9]{11}/);
  }
  ...
  async add(media, interaction) {
    ...
    if (media.type === 'playlist') {
      messageContent = `added **${media.videoCount}** videos from [**${media.title}**](${media.url}) to queue`;
      ...
      messageContent = `added [**${media.title}**](${media.url}) to queue`;
      videoArray = [media];
    }
  ...
  async play(interaction, input, mediaType) {
    const inVoice = this.checkUserVoice(interaction);
    if (!inVoice) return;

    const songs = await this._getSongs(input);
  ...
  checkUserVoice(interaction) {
    const inVoice = interaction.member.voice.channel;
  ...
  remove(index, interaction) {
    ...
    interaction.reply(
      `removed **${isArray ? `${removed.length} songs` : removed.title}** from queue`
    );
  }
